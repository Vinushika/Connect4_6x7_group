 ...
 * <p>
 *
 * Theoretical design of BTreeMap is based on 1986 paper <a
 * href="http://www.sciencedirect.com/science/article/pii/0022000086900218">
 * Concurrent operations on B∗-trees with overtaking</a> written by Yehoshua
 * Sagiv. More practical aspects of BTreeMap implementation are based on <a
 * href="http://cs.au.dk/~tyoung/~td202/">notes</a> and <a
 * href="http://cs.au.dk/~tyoung/~td202/btree/">demo application</a> from Thomas
 * Dinsdale-Young. Also more work from Thomas: <a
 * href="http://www.doc.ic.ac.uk/research/technicalreports/2011/#10">A Simple
 * Abstraction for Complex Concurrent Indexes</a>
 * </p>
 * <p>
 *
 * B-Linked-Tree used here does not require locking for read. Updates and
 * inserts locks only one, two or three nodes. Original BTree design does not
 * use overlapping lock (lock is released before parent node is locked), I added
...
public class BTreeMap<K, V> extends AbstractMap<K, V> implements ConcurrentNavigableMap<K, V>, Bind.MapWithModificationListener<K, V>, Closeable {
	...
	/**
	 * Indicates if this collection collection was not made by DB by user. If
	 * user can not access DB object, we must shutdown Executor and close Engine
	 * ourself in close() method.
	 */
	...
	protected static SortedMap<String, Object> preinitCatalog(DB db) {
		...
		return new BTreeMap<String, Object>(db.engine, false, Engine.RECID_NAME_CATALOG, 32, false, 0, keyser, valser, 0);
	}
	...
	public abstract static class BNode {
		...
		public BNode(Object keys, boolean leftEdge, boolean rightEdge, boolean tooLarge) {
			...
			this.flags = (byte) ((leftEdge ? LEFT_MASK : 0) | (rightEdge ? RIGHT_MASK : 0) | (tooLarge ? TOO_LARGE_MASK : 0));
		}
		...
		public final int compare(final BTreeKeySerializer keyser, int pos1, int pos2) {
			...
			// $DELAY$
			if (isLeftEdge()) {
				// first position is negative infinity, so everything else is
				// bigger
			...
			if (isRightEdge()) {
				...
				// last position is positive infinity, so everything else is
				// smaller
				if (pos1 == keysLen)
					return 1;
				if (pos2 == keysLen)
	...
	public final static class LeafNode extends BNode {
		...
		@Override
		public String toString() {
			String valsStr = Fun.toString(vals); // TODO use value serializer to
													// turn this into string
	...
	protected static final class NodeSerializer<A, B> extends Serializer<BNode> {
		...
		public NodeSerializer(boolean valsOutsideNodes, BTreeKeySerializer keySerializer, Serializer valueSerializer, int numberOfNodeMetas) {
			...
			this.valueSerializer = hasValues ? (valsOutsideNodes ? new ValRefSerializer() : valueSerializer) : Serializer.BOOLEAN;
		...
		public void serialize(DataOutput out, BNode value) throws IOException {
			...
			final int header = (isLeaf ? LEAF_MASK : 0) | (value.isLeftEdge() ? LEFT_MASK : 0) | (value.isRightEdge() ? RIGHT_MASK : 0) | value.keysLen(keySerializer);
		...
		protected void serializeChildArray(DataOutput out, Object childArray) throws IOException {
			if (childArray instanceof int[]) {
				...
				DataIO.packLong(out, (((long) cc[0]) << 1) | 1L); // pack first
																	// value
																	// mixed
																	// with int
																	// flag
				...
				DataIO.packLong(out, (cc[0] << 1) | 0); // pack first value
														// mixed with int flag
				for (int i = 1; i < cc.length; i++) {
		...
		public BNode deserialize(DataInput in, int available) throws IOException {
			...
			DataIO.DataInputInternal in2 = (DataIO.DataInputInternal) in; // TODO
																			// fallback
																			// option
																			// if
																			// cast
	 ...
	 * @param closeEngine
	 *            if this object was created without DB. If true shutdown
	 *            everything on close method, otherwise DB takes care of
	 *            shutdown
	 * @param rootRecidRef
	 ...
	 * @param counterRecid
	 *            recid under which {@code Atomic.Long} is stored, or {@code 0}
	 *            for no counter
	 * @param keySerializer
	 ...
	 */
	public BTreeMap(Engine engine, boolean closeEngine, long rootRecidRef, int maxNodeSize, boolean valsOutsideNodes, long counterRecid, BTreeKeySerializer keySerializer,
			final Serializer<V> valueSerializer, int numberOfNodeMetas) {
		...
		// SerializerBase.assertSerializable(keySerializer); //TODO serializer
		// serialization
	...
	static protected long createRootRef(Engine engine, BTreeKeySerializer keySer, Serializer valueSer, int numberOfNodeMetas) {
		Object emptyArray = valueSer != null ? valueSer.valueArrayEmpty() : Serializer.BOOLEAN.valueArrayEmpty();
		...
		try {
			while (true) {
				...
				do {
					...
					// $DELAY$
					if (pos < A.keysLen(keySerializer) - 1 && v != null && A.key(keySerializer, pos) != null && // TODO
																												// A.key(pos]!=null??
				...
				if (A.keysLen(keySerializer) - (A.isLeaf() ? 1 : 0) < maxNodeSize) {
					...
					if ((current != rootRecid)) { // is not root
						...
						Object rootChild = (current < Integer.MAX_VALUE && q < Integer.MAX_VALUE) ? new int[] { (int) current, (int) q, 0 } : new long[] { current, q, 0 };

						BNode R = new DirNode(keySerializer.arrayToKeys(new Object[] { A.highKey(keySerializer) }), true, true, false, rootChild);
						// $DELAY$
		...
		private void pointToStart() {
			...
			for (;;) {
				...
				if (next == 0) {
					...
					next = children instanceof int[] ? ((int[]) children)[((int[]) children).length - 2] : ((long[]) children)[((long[]) children).length - 2];
				}
		...
		try {
			for (;;) {
				...
				if (pos > 0 && pos != A.keysLen(keySerializer) - 1) {
					...
					A = putNewValue != null ? ((LeafNode) A).copyChangeValue(valueSerializer, pos, putNewValueOutside) : ((LeafNode) A).copyRemoveKey(keySerializer, valueSerializer, pos);
					if (CC.PARANOID && !(nodeLocks.get(current) == Thread.currentThread()))
						throw new AssertionError();
					// $DELAY$
					engine.update(current, A, nodeSerializer);
	...
	private Entry<K, V> findSmallerRecur(BNode n, K key, boolean inclusive) {
		...
		for (int i = start; i >= end; i--) {
			...
			if (comp < res) {
				if (leaf) {
					// $DELAY$
					return key2 == null ? null : makeEntry(key2, valExpand(n.val(i - 1, valueSerializer)));
				} else {
					...
					if (n2.isLeaf()) {
						...
						if (n2.keysLen(keySerializer) > 2 && keySerializer.comparator().compare(n2.key(keySerializer, 1), key) >= (inclusive ? 1 : 0)) {
							continue;
						}
					}
					// $DELAY$
	...
	static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E>, Closeable {
	...
	static protected class SubMap<K, V> extends AbstractMap<K, V> implements ConcurrentNavigableMap<K, V> {
		...
		@Override
		public int size() {
			// TODO add method which returns long, compatible with new method in
			// Java8 streams, not forget other submaps, reverse maps
	...
	public NavigableMap<K, V> snapshot() {
		...
		return new BTreeMap<K, V>(snapshot, closeEngine, rootRecidRef, maxNodeSize, valsOutsideNodes, counter == null ? 0L : counter.recid, keySerializer, valueSerializer, numberOfNodeMetas);
	}
