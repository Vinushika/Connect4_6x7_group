...
public class Connect4Control implements InterfaceControl, ThreadCompleteListener {
	...
	private InterfaceStrategy player1Strategy = null;// new
														// Connect4Strategy();//
	private InterfaceStrategy player2Strategy = null;// new
														// Connect4Strategy();//
	...
	@Override
	public void onMove() { // Control is notified of a player (real or
							// automated) move
		...
		context.setMaxSearchTimeForThisPos(2000000000L);// 2 seconds (in
														// nanoseconds) from now
		context.setMaxDepthSearchForThisPos(15); // Search no more than ## moves
													// deep
		if (currentPlayer == 1) {
			if (player1Strategy != null && !isBlockManualMove()) { // isStrategy
																	// and game
																	// not over
				context.setMinDepthSearchForThisPos(preferedMinDepthPlayer1); // Search
																				// no
																				// less
																				// than
																				// ##
		...
		if (currentPlayer == 2) {
			if (player2Strategy != null && !isBlockManualMove()) { // isStrategy
																	// and game
																	// not over
				context.setMinDepthSearchForThisPos(preferedMinDepthPlayer2); // Search
																				// no
																				// less
																				// than
																				// ##
	...
	public void onMove(int unused1, int unused2, int iColumn, int iRow, int iColor) {
		InterfaceIterator iter = new Connect4Iterator(view.nC(), view.nR());
		iter.set(iColumn, iRow);
	...
	public void setStrategy(int player, int strategy) {
		InterfaceStrategy playerStrategy = strategy == 0 ? null : strategy == 1 ? new Connect4StrategyDalen() : new Connect4StrategyB();
		if (player == 1)
	...
	public void notifyOfThreadComplete(Thread thread) {
		...
		view.performMove(0, 0, iC, iR, 0); // 0 references is not used in this
											// view

		InterfaceSearchContext context = ((ThreadStrategy) thread).getContext();
		if (((Connect4SearchContext) context).getOriginalPlayer() == 1) { // TODO:
																			// fix
																			// Connect4SearchContext
																			// cast
